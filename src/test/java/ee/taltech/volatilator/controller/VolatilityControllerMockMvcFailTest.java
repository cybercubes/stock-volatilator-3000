...
public class VolatilityControllerMockMvcFailTest {
    ...
    void volatilityController_incorrect_date_format_fail() throws Exception {
        mvc.perform(get("/volatilator?symbol=dateFormatFail&startDate=aaaaa").accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
    ...
    void volatilityController_start_date_after_end_date_fail() throws Exception {
        mvc.perform(get("/volatilator?symbol=dateOrderFail&startDate=2020-01-01&endDate=2019-01-01").accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("fail"))
    ...
    void volatilityController_day_range_is_less_than_100_fail() throws Exception {
        mvc.perform(get("/volatilator?symbol=dateIntervalFail&startDate=2019-01-01&endDate=2020-01-01").accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("fail"))
